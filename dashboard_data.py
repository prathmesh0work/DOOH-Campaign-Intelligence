...
def build_tables(rows):
    ...
    summaries.sort(key=lambda c: -c["roi"])  # best ROI first

    top_5 = summaries[:5]
    bottom_5 = list(reversed(summaries[-5:]))  # worst ROI first
...
def build_screen_table(rows):
    ...
    for screen_id, group_rows in by_screen.items():
        if screen_id == "Unknown Screen":
            continue  # skip rows where we couldn't detect a screen ID
        ...
        screens.append({
            "screen_id": screen_id,
            "city": group_rows[0].get("City", "Unknown"),   # a screen sits in one city
            "revenue": revenue,
...
def build_discrepancy_report(rows):
    ...
    for row in rows:
        ...
        if committed <= 0:
            continue  # this row never had a commitment recorded — nothing to compare
    ...
    flagged.sort(key=lambda f: -f["gap_percent"])  # worst under-delivery first
    return flagged
...
def build_anomaly_report(rows):
    ...
    for screen_id, screen_rows in by_screen.items():
        ...
        for day, day_rows in by_date.items():
            if day:  # skip rows with no valid date
                daily_totals[day] = sum_column(day_rows, "Impressions")

        if len(daily_totals) < MIN_ACTIVE_DAYS_TO_JUDGE:
            continue  # not enough history to know what's "normal" for this screen
    ...
    anomalies.sort(key=lambda a: -a["drop_percent"])  # worst drop first
    return anomalies
